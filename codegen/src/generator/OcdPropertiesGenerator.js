...
export const ${this.reactObjectListName(attribute.name)} = ({ ocdDocument, setOcdDocument, resource, configs }: GeneratedResourceProperties): JSX.Element => {
    const onClick = (e: React.MouseEvent<HTMLElement>) => {
        e.stopPropagation()
        e.preventDefault()
        resource.push(Model.${this.modelNamespaceName(resource)}.${this.modelNamespaceFunctionName(attribute.name)}())
        setOcdDocument(OcdDocument.clone(ocdDocument))
    }
    const onDelete = (child: Model.${this.modelInterfaceName(attribute.name)}) => {
        resource.splice(resource.indexOf(child), 1)
        setOcdDocument(OcdDocument.clone(ocdDocument))
    }
    resource.forEach((r: Model.OciRouteTableModel) => {if (!r.key) r.key = uuidv4()})
    return (
        <div className='ocd-property-row'>
            <details open={true}>
                ...
                <div className='ocd-resource-properties'>
                    {resource.map((r: Model.${this.modelInterfaceName(resource)}) => {return <${this.reactObjectName(attribute.name)} ocdDocument={ocdDocument} setOcdDocument={(ocdDocument:OcdDocument) => setOcdDocument(ocdDocument)} resource={r} configs={configs} onDelete={onDelete} key={r.key}/>})}
                </div>
            </details>
        </div>
    )
    ...
    resourceFilename = (resource) => `${this.resourcePropertiesFilename(resource)}.tsx`
