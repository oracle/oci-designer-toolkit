...
import { ${this.autoGeneratedInterfaceName(resource)} } from "./${this.generatedDirectory()}/${this.interfaceName(resource)}"

export interface ${this.interfaceName(resource)} extends ${this.autoGeneratedInterfaceName(resource)} {
}

export namespace ${this.namespaceName(resource)} {
    export function newResource(type?: string): ${this.interfaceName(resource)} {
        return {
            ...${this.autoGeneratedNamespaceName(resource)}.newResource('${resource}'),
        }
    }
}
...
`
    ...
    autoGeneratedClassName = (resource) => `${this.autoGeneratedResourceeName(resource)}Client`

    interfaceName = (resource) => this.resourceName(resource)
    autoGeneratedInterfaceName = (resource) => this.autoGeneratedResourceeName(resource)

    namespaceName = (resource) => this.resourceName(resource)
    autoGeneratedNamespaceName = (resource) => this.autoGeneratedResourceeName(resource)
}
