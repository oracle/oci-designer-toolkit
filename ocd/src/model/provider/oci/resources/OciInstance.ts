/*
** Copyright (c) 2020, 2023, Oracle and/or its affiliates.
** Licensed under the Universal Permissive License v 1.0 as shown at https://oss.oracle.com/licenses/upl.
*/

import * as AutoGenerated from "./generated/OciInstance"

export interface OciInstance extends AutoGenerated.OciInstance {}

export interface OciAgentConfig extends AutoGenerated.OciAgentConfig {}

export interface OciPluginsConfig extends AutoGenerated.OciPluginsConfig {}

export interface OciAvailabilityConfig extends AutoGenerated.OciAvailabilityConfig {}

export interface OciCreateVnicDetails extends AutoGenerated.OciCreateVnicDetails {}

export interface OciInstanceOptions extends AutoGenerated.OciInstanceOptions {}

export interface OciLaunchOptions extends AutoGenerated.OciLaunchOptions {}

export interface OciPlatformConfig extends AutoGenerated.OciPlatformConfig {}

export interface OciPreemptibleInstanceConfig extends AutoGenerated.OciPreemptibleInstanceConfig {}

export interface OciPreemptionAction extends AutoGenerated.OciPreemptionAction {}

export interface OciShapeConfig extends AutoGenerated.OciShapeConfig {}

export interface OciSourceDetails extends AutoGenerated.OciSourceDetails {}

export namespace OciInstance {
    export function newResource(type?: string): OciInstance {
        return {
            ...AutoGenerated.OciInstance.newResource('instance'),
        }
    }
    
    export function newOciAgentConfig(): OciAgentConfig {
        return {
            ...AutoGenerated.OciInstance.newOciAgentConfig(),
        }
    }

    export function newOciPluginsConfig(): OciPluginsConfig {
        return {
            ...AutoGenerated.OciInstance.newOciPluginsConfig(),
        }
    }

    export function newOciAvailabilityConfig(): OciAvailabilityConfig {
        return {
            ...AutoGenerated.OciInstance.newOciAvailabilityConfig(),
        }
    }

    export function newOciCreateVnicDetails(): OciCreateVnicDetails {
        return {
            ...AutoGenerated.OciInstance.newOciCreateVnicDetails(),
        }
    }

    export function newOciInstanceOptions(): OciInstanceOptions {
        return {
            ...AutoGenerated.OciInstance.newOciInstanceOptions(),
        }
    }

    export function newOciLaunchOptions(): OciLaunchOptions {
        return {
            ...AutoGenerated.OciInstance.newOciLaunchOptions(),
        }
    }

    export function newOciPlatformConfig(): OciPlatformConfig {
        return {
            ...AutoGenerated.OciInstance.newOciPlatformConfig(),
        }
    }

    export function newOciPreemptibleInstanceConfig(): OciPreemptibleInstanceConfig {
        return {
            ...AutoGenerated.OciInstance.newOciPreemptibleInstanceConfig(),
        }
    }

    export function newOciPreemptionAction(): OciPreemptionAction {
        return {
            ...AutoGenerated.OciInstance.newOciPreemptionAction(),
        }
    }

    export function newOciShapeConfig(): OciShapeConfig {
        return {
            ...AutoGenerated.OciInstance.newOciShapeConfig(),
        }
    }

    export function newOciSourceDetails(): OciSourceDetails {
        return {
            ...AutoGenerated.OciInstance.newOciSourceDetails(),
        }
    }

}

export class OciInstanceClient {
    static new(): OciInstance {
        return OciInstance.newResource()
    }
}

export default OciInstanceClient
