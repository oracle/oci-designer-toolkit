...
export namespace AzureResourceGroupProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.AzureResourceGroup, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.AzureResourceGroup>(resource, pH)
