...
export namespace AzureSubnetProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.AzureSubnet, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.AzureSubnet>(resource, pH)
