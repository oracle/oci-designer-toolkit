...
export namespace OciServiceMeshProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciServiceMesh, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciServiceMesh>(resource, pH)
