...
export namespace OciApiGatewayProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciApiGateway, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciApiGateway>(resource, pH)
