...
export namespace OciHealthChecksPingProbeProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciHealthChecksPingProbe, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciHealthChecksPingProbe>(resource, pH)
