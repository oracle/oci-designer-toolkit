...
export namespace OciEmailSenderProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciEmailSender, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciEmailSender>(resource, pH)
