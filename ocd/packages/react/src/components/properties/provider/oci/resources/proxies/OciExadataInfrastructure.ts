...
export namespace OciExadataInfrastructureProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciExadataInfrastructure, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciExadataInfrastructure>(resource, pH)
