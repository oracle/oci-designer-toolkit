...
export namespace OciPublicIpProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciPublicIp, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciPublicIp>(resource, pH)
