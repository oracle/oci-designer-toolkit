...
export namespace OciEmailDomainProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciEmailDomain, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciEmailDomain>(resource, pH)
