...
export namespace ${this.idToNamespaceName(resource)} {
    export function validateResource(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): OcdValidationResult[] {
    ...
    export function isResourceValid(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): boolean {
        return (validateResource(resource, resources).filter((v: OcdValidationResult) => !v.valid).length > 0)
    }
    export function hasErrors(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): boolean {
        return (validateResource(resource, resources).filter((v: OcdValidationResult) => v.type === 'error').length > 0)
    }
    export function hasWarnings(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): boolean {
        return (validateResource(resource, resources).filter((v: OcdValidationResult) => v.type === 'warning').length > 0)
    }
    export function hasInformation(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): boolean {
        return (validateResource(resource, resources).filter((v: OcdValidationResult) => v.type === 'information').length > 0)
    }
    function customValidation(resource: Model.${this.idToInterfaceName(resource)}, resources: OciResources): OcdValidationResult[] {
    ...
    */
    ...
    validatorSimpleElement = (resource, attribute, level=0) => {
        ...
        if (simpleTypes.includes(attribute.type) || (groupTypes.includes(attribute.type) && simpleTypes.includes(attribute.subtype))) {
            return `const ${this.validatorSimpleName(attribute.id)} = (resource: OcdValidatorResource, resources: OciResources, displayName=''): OcdValidationResult[] => {return [${this.validatorSimpleElementType(resource, attribute.name, attribute)}]}`
        } else {
            return `const ${this.validatorSimpleName(attribute.id)} = (resource: OcdValidatorResource, resources: OciResources, displayName=''): OcdValidationResult[] => {return [${this.validatorSimpleElementType(resource, attribute.name, attribute)}]}`
        }
    }
    ...
    validatorObjectElement = (resource, attribute, level=0) => {
        ...
        return `const ${this.validatorObjectName(attribute.id)} = (resource: OcdValidatorResource | undefined, resources: OciResources, displayName=''): OcdValidationResult[] => {
    ...
    const ${this.validatorObjectListName(attribute.id)} = (resource: OcdValidatorResource[] | undefined, resources: OciResources, displayName=''): OcdValidationResult[] => {
