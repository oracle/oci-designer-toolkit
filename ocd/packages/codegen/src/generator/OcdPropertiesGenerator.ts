...
export class OcdPropertiesGenerator extends OcdCodeGenerator {
    propertySortSequence: OcdPropertySortSequence = {}
    sortSequence: string[] = []
    objectTypes = ['object']
    ...
    generate(resource, schema) {
        const tfResource = schema.tf_resource
        this.sortSequence = this.propertySortSequence[tfResource] ?? []
    ...
    writeFiles(outputDirectory, resource, force = false) {
        ...
        if (force || !fs.existsSync(configFilename)) {
        ...
        if (force || !fs.existsSync(proxyFilename)) {
...
import * as AutoGenerated from './${this.generatedDirectory()}/${this.reactResourceName(resource)}.js'
import { ${this.configNamespace(resource)} } from './${this.configsDirectory()}/${this.reactResourceName(resource)}.js'
...
`
    ...
    */
    autoGeneratedContent = (resource, schema) => {
        ...
        const schemaAttributesMap = this.sortAttributes(schema.attributes)
        // console.debug("OcdPropertiesGenerator: schemaAttributeMap Entries", resource, schemaAttributesMap.entries())
        const content = `${this.copyright()}
...
export const ${this.reactResourceGeneratedName(resource)} = ({ ocdDocument, setOcdDocument, resource, configs, additionalElements = [], summaryTitle = 'Resource', onDelete = undefined }: GeneratedResourceRootProperties): JSX.Element => {
    ...
    const className = onDelete === undefined ? \`\${summaryClassNames}\` : \`\${summaryClassNames} ocd-summary-row\`
        ...
        <div>
            <details open={true} onToggle={onToggle}>
                ...
                <div className='ocd-resource-properties'>
                    ${Array.from(schemaAttributesMap).map(([k, a]) => this.attributeToReactElement(resource, a)).join('\n                    ')}
                </div>
            </details>
        </div>
    )
...
`
    ...
    sortAttributes = (attributes): Map<string, any> => {
        if (this.sortSequence.length > 0) {
            const attributesMap = new Map()
            this.sortSequence.filter((ss) => !this.ignoreAttributes.includes(ss)).forEach((ss) => attributesMap.set(ss, attributes[ss]))
            // console.debug('Attribute Map:', attributesMap)
            return attributesMap
        } else {
            return new Map(Object.entries(attributes).filter(([k, v]) => !this.ignoreAttributes.includes(v.id)))
        }
    }
...
import { ${this.prefix}CommonConfigs } from "../../${this.prefix}CommonConfigs.js"

export namespace ${this.configNamespace(resource)} {
    export function configs(): ResourceElementConfig[] {return [...${this.prefix}CommonConfigs.configs()]}
}
`
        return content
    }
...
${resources.sort(this.simpleSort).map((r) => `export { ${this.proxyNamespace(r)} } from './${this.resourcesDirectory()}/proxies/${this.resourceName(r)}.js'`).join('\n')}
    ...
    reactSimpleElementType = (resource, name, attribute) => {
        ...
        else if (attribute.type === 'number' && attribute.staticLookup)       return `<OcdStaticLookupProperty ocdDocument={ocdDocument} setOcdDocument={(ocdDocument:OcdDocument) => setOcdDocument(ocdDocument)} resource={resource} config={${configFind}} attribute={${JSON.stringify(attribute)}} rootResource={rootResource} key={\`OcdProperty-\${resource.id ? resource.id : resource.key ? resource.key : rootResource.id}-${attribute.id}\`} />`
