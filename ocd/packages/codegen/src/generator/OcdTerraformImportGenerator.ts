...
export class OciResourceBuilder extends OcdResourceBuilder {
    resourceMap: Record<string, string> = ${JSON.stringify(this.resourceMap, null, 12)}
...
//     }
    ...
    ${this.terraformObjectName(attribute.id)} = (resource: Record<string, any>, ocdResource: Record<string, any>, level=0): ${this.generateModelHierarchyReference(resource, attribute.id)} => {
        ocdResource.${this.toCamelCase(attribute.name)}  = this.${this.terraformObjectName(createFunction)}(resource, ocdResource)
