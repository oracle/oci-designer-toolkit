...
export interface ${this.autoGeneratedInterfaceName(resource)} extends ${this.prefix}Resource {
    ${Object.entries(schema.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(v.id)).map(([k, a]) => this.interfaceAttributeDefinition(k, a)).join('\n    ')}
}
...
export namespace ${this.autoGeneratedNamespaceName(resource)} {
    ...
    export function newResource(type: string): ${this.autoGeneratedInterfaceName(resource)} {
        return {
            ...
            ${Object.entries(schema.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(v.id)).map(([k, a]) => this.namespaceAttributeAssignment(resource, k, a)).join(',\n            ')}
        }
    }
...
export interface ${this.interfaceName(i.name)} {
    ${Object.entries(i.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(v.id)).map(([k, a]) => this.interfaceAttributeDefinition(k, a)).join('\n    ')}
}
`
    }
...
`
    ...
    export function ${this.namespaceFunctionName(i.name)}(): ${this.interfaceName(i.name)} {
        return {
            ${Object.entries(i.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(v.id)).map(([k, a]) => this.namespaceAttributeAssignment(resource, k, a)).join(',\n            ')}
        }
    }
`
    }
