...
//     }
    ...
    interfaceAttributeDefinition = (attribute, level: number=0) => {
        ...
        if (attribute.type === 'string')      return `${this.toCamelCase(name)}${requiredString}: string`
        else if (attribute.type === 'bool')   return `${this.toCamelCase(name)}${requiredString}: boolean`
        else if (attribute.type === 'number') return `${this.toCamelCase(name)}${requiredString}: number`
        else if (attribute.type === 'object') return `${this.toCamelCase(name)}${requiredString}: ${this.generateInterfaceReference(name, level)}`
        // else if (attribute.type === 'object') return `${this.toCamelCase(name)}${requiredString}: ${this.interfaceName(attribute.id)}`
        else if (attribute.type === 'list')   return `${this.toCamelCase(name)}${requiredString}: ${attribute.subtype === 'object' ? this.generateInterfaceReference(name, level) : attribute.subtype}[]`
        // else if (attribute.type === 'list')   return `${this.toCamelCase(name)}${requiredString}: ${attribute.subtype === 'object' ? this.interfaceName(attribute.id) : attribute.subtype}[]`
        else if (attribute.type === 'set')    return `${this.toCamelCase(name)}${requiredString}: ${attribute.subtype}[]`
        else if (attribute.type === 'map')    return `${this.toCamelCase(name)}${requiredString}: {[key: string]: string}`
        else return ''
    }
