...
`
    ...
    attributeToTerraformAssignment = (resource, name, attribute, level=0) => {
        ...
        else if (attribute.type === 'object')                                 return `\${this.isPropertyAssignConditionTrue(${attribute.conditional}, ${JSON.stringify(attribute.condition)}, resource) ? this.${this.terraformObjectName(attribute.name)}(resource.${this.modelElementName(attribute.name)}${level === 0 ? '' : ', level+1'}) : ''}`
    ...
    terraformSimpleElementType = (resource, name, attribute, level=0) => {
        if (attribute.type === 'string' && attribute.staticLookup)            return `this.isPropertyAssignConditionTrue(${attribute.conditional}, ${JSON.stringify(attribute.condition)}, resource) ? this.generateTextAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'string' && attribute.lookup)             return `this.generateReferenceAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level)`
    ...
    ${this.terraformObjectName(attribute.name)} = (resource: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)} | undefined, level=0): string => {
            return resource && this.${this.terraformObjectAssignmentTestName(attribute.name)}(resource) ? \`\${this.indentation[level]}${attribute.name} {
