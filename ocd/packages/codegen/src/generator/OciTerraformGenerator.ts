...
`
    ...
    attributeToAssignmentTest = (resource, name, attribute) => {
        if (attribute.type === 'string' && attribute.lookup)                  return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else if (attribute.type === 'string')                                 return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else if (attribute.type === 'bool')                                   return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else if (attribute.type === 'number')                                 return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else if (attribute.type === 'list' && attribute.subtype === 'string') return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else if (attribute.type === 'object')                                 return `this.${this.terraformObjectAssignmentTestName(attribute.name)}(resource.${this.modelElementName(attribute.name)}, rootResource)`
        else if (attribute.type === 'object')                                 return `this.${this.terraformObjectAssignmentTestName(attribute.name)}(resource.${this.modelElementName(attribute.name)}, rootResource)`
        else if (attribute.type === 'list' && attribute.subtype === 'object') return `this.${this.terraformObjectListAssignmentTestName(attribute.name)}(resource.${this.modelElementName(attribute.name)}, rootResource)`
        else if (attribute.type === 'list' && attribute.lookup)               return `this.${this.terraformSimpleAssignmentTestName(attribute.name)}(resource, rootResource)`
        else return `false`
    }

    attributeToTerraformAssignment = (resource, name, attribute, level=0) => {
        if (attribute.type === 'string' && attribute.lookup)                  return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'string')                                 return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'bool')                                   return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'number')                                 return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'list' && attribute.subtype === 'string') return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'list' && attribute.lookup)               return `\${this.${this.terraformSimpleName(attribute.name)}(resource${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'object')                                 return `\${this.${this.terraformObjectName(attribute.name)}(resource.${this.modelElementName(attribute.name)}${level === 0 ? ', resource' : ', rootResource, level+1'})}`
        else if (attribute.type === 'list' && attribute.subtype === 'object') return `\${this.${this.terraformObjectListName(attribute.name)}(resource.${this.modelElementName(attribute.name)}${level === 0 ? ', resource' : ', rootResource, level+1'})}`
    ...
    terraformSimpleElementType = (resource, name, attribute, level=0, resourceName='common') => {
        ...
        if (Object.hasOwn(mentadataAttributes, name))                         return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateMetadataAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, '${mentadataAttributes[name]}', level) : ''`
        else if (attribute.type === 'string' && attribute.cacheLookup)        return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateCacheAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level, '${attribute.lookupResource}') : ''`
        else if (attribute.type === 'string' && attribute.staticLookup)       return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateTextAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'string' && attribute.lookup)             return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateReferenceAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level${attribute.lookupResourceElement && attribute.lookupResourceElement !== '' ? `, '${attribute.lookupResourceElement}'` : ''}) : ''`
        else if (attribute.type === 'string')                                 return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateTextAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'bool')                                   return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateBooleanAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'number')                                 return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateNumberAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'list' && attribute.lookup)               return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateReferenceListAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'list' && attribute.subtype === 'string') return `this.${this.terraformSimpleAssignmentTestName(name)}(resource, rootResource) ? this.generateStringListAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
    ...
    ${this.terraformSimpleName(attribute.name)} = (resource: Record<string, any>, rootResource: OciResource, level=0): string => {return ${this.terraformSimpleElementType(resource, attribute.name, attribute)}}
    ${this.terraformSimpleAssignmentTestName(attribute.name)} = (resource: Record<string, any>, rootResource: OciResource): boolean => {return ${this.simpleAttributeAssignmentTest(resource, attribute.name, attribute)}}
    `
    ...
    ${this.terraformObjectName(attribute.name)} = (resource: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)} | undefined, rootResource: OciResource, level=0): string => {
            return resource && this.${this.terraformObjectAssignmentTestName(attribute.name)}(resource, rootResource) ? \`\${this.indentation[level]}${attribute.name}${attribute.subtype === 'map' ? ' =' : ''} {
    ...
    ${this.terraformObjectAssignmentTestName(attribute.name)} = (resource: Record<string, any>, rootResource: OciResource): boolean => {
        return this.isPropertyAssignConditionTrue(${attribute.conditional}, ${JSON.stringify(attribute.condition)}, resource, rootResource) && (${Object.keys(attribute.attributes).filter((k) => !this.ignoreAttributes.includes(k)).length > 0 ? Object.entries(attribute.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(k)).map(([k, a]) => this.attributeToAssignmentTest(resource, k, a)).join(' || ') : 'false'})
    }
    `
    }
    ...
    ${this.terraformObjectListName(attribute.name)} = (objectList: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)}[] | undefined, rootResource: OciResource, level=0): string => {
        return objectList && objectList.length > 0 ? \`\${objectList.map((o: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)}) => this.${this.terraformObjectName(attribute.name)}(o, rootResource)).join(\`\\n\${this.indentation[level+1]}\`)}\` : '# ${attribute.name} is not defined. Type: ${attribute.type} SubType: ${attribute.subtype} Required: ${attribute.required}'
    }
    ${this.terraformObjectListAssignmentTestName(attribute.name)} = (objectList: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)}[] | undefined, rootResource: OciResource): boolean => {
        return objectList && objectList.length > 0 ? objectList.some((o: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)}) => this.${this.terraformObjectAssignmentTestName(attribute.name)}(o, rootResource)) : false
    }
    `
