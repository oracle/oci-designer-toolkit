...
`
    ...
    ${this.terraformObjectAssignmentTestName(attribute.name)} = (resource: Record<string, any>): boolean => {
        return this.isPropertyAssignConditionTrue(${attribute.conditional}, ${JSON.stringify(attribute.condition)}, resource) && (${Object.keys(attribute.attributes).filter((k) => !this.ignoreAttributes.includes(k)).length > 0 ? Object.entries(attribute.attributes).filter(([k, v]) => !this.ignoreAttributes.includes(k)).map(([k, a]) => this.attributeToAssignmentTest(resource, k, a)).join(' || ') : 'false'})
    }
    `
    }
