...
`
    ...
    attributeToTerraformAssignment = (resource, name, attribute, level=0) => {
        ...
        else if (attribute.type === 'object')                                 return `\${this.${this.terraformObjectAssignmentTestName(attribute.name)}(resource.${this.modelElementName(attribute.name)}) ? this.${this.terraformObjectName(attribute.name)}(resource.${this.modelElementName(attribute.name)}${level === 0 ? '' : ', level+1'}) : ''}`
        else if (attribute.type === 'list' && attribute.subtype === 'object') return `\${this.${this.terraformObjectListAssignmentTestName(attribute.name)}(resource.${this.modelElementName(attribute.name)}) ? this.${this.terraformObjectListName(attribute.name)}(resource.${this.modelElementName(attribute.name)}${level === 0 ? '' : ', level+1'}) : ''}`
    ...
    terraformSimpleElementType = (resource, name, attribute, level=0, resourceName='common') => {
        ...
        if (Object.hasOwn(mentadataAttributes, name))                         return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateMetadataAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, '${mentadataAttributes[name]}', level) : ''`
        else if (attribute.type === 'string' && attribute.cacheLookup)        return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateCacheAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level, '${attribute.lookupResource}') : ''`
        else if (attribute.type === 'string' && attribute.staticLookup)       return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateTextAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'string' && attribute.lookup)             return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateReferenceAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level${attribute.lookupResourceElement && attribute.lookupResourceElement !== '' ? `, '${attribute.lookupResourceElement}'` : ''}) : ''`
        else if (attribute.type === 'string')                                 return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateTextAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'bool')                                   return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateBooleanAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'number')                                 return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateNumberAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'list' && attribute.lookup)               return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateReferenceListAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
        else if (attribute.type === 'list' && attribute.subtype === 'string') return `this.${this.terraformSimpleAssignmentTestName(name)}(resource) ? this.generateStringListAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level) : ''`
