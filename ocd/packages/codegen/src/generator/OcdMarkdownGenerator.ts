...
`
    ...
    markdownSimpleElementType = (resource, name, attribute, level=0, resourceName='common') => {
        ...
        if (attribute.type === 'list' && attribute.lookup)                    return `this.generateReferenceListAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level, this.allResources)`
