...
export class AzureTerraformGenerator extends OcdCodeGenerator {
    constructor () {
        super()
...
import { AzureTerraformResource } from '../../AzureTerraformResource'
import { AzureModelResources as Model, OcdDesign, AzureResource } from '@ocd/model'

export class ${this.autoGeneratedResourceName(resource)} extends AzureTerraformResource {
...
`
    ...
    terraformSimpleElementType = (resource, name, attribute, level=0, resourceName='common') => {
        ...
        else if (attribute.type === 'string' && attribute.lookup)             return `this.${this.terraformSimpleAssignmentTestName(attribute.id)}(resource, rootResource) ? this.generateReferenceAttribute("${name}", resource.${this.toCamelCase(name)}, ${attribute.required}, level${lookupString}) : ''`
    ...
    ${this.terraformSimpleName(attribute.id)} = (resource: Record<string, any>, rootResource: AzureResource, level=0): string => {return ${this.terraformSimpleElementType(resource, attribute.name, attribute)}}
    ${this.terraformSimpleAssignmentTestName(attribute.id)} = (resource: Record<string, any>, rootResource: AzureResource): boolean => {return ${this.simpleAttributeAssignmentTest(resource, attribute.name, attribute)}}
    `
    ...
    terraformObjectElement = (resource, attribute, level=0) => {
        ...
        // ${this.terraformObjectName(attribute.name)} = (resource: Model.${this.interfaceName(resource)}.${this.interfaceName(attribute.name)} | undefined, rootResource: AzureResource, level=0): string => {
        return `// ${OcdUtils.toResourceTypeName(attribute.name)} (${this.terraformObjectName(attribute.name)})
    ${this.terraformObjectName(attribute.id)} = (resource: ${this.generateModelHierarchyReference(resource, attribute.id)} | undefined, rootResource: AzureResource, level=0): string => {
    ...
    ${this.terraformObjectAssignmentTestName(attribute.id)} = (resource: ${this.generateModelHierarchyReference(resource, attribute.id)} | undefined, rootResource: AzureResource): boolean => {
    ...
    ${this.terraformObjectListName(attribute.id)} = (objectList: ${this.generateModelHierarchyReference(resource, attribute.id)}[] | undefined, rootResource: AzureResource, level=0): string => {
    ...
    ${this.terraformObjectListAssignmentTestName(attribute.id)} = (objectList: ${this.generateModelHierarchyReference(resource, attribute.id)}[] | undefined, rootResource: AzureResource): boolean => {
