...
export namespace OcdConfigFacade {
    export const loadConsoleConfig = (): Promise<any> => {
        return window.ocdAPI ? window.ocdAPI.loadConsoleConfig() : Promise.reject('Currently Not Implemented')
    }
    export const saveConsoleConfig = (config: OcdConsoleConfiguration): Promise<any> => {
        return window.ocdAPI ? window.ocdAPI.saveConsoleConfig(config) : Promise.reject('Currently Not Implemented')
    }
}
