...
export namespace OcdConfigFacade {
    export const loadConsoleConfig = (): Promise<any> => {
        return window.ocdAPI ? window.ocdAPI.loadConsoleConfig() : new Promise((resolve, reject) => {reject('Currently Not Implemented')})
    }
    export const saveConsoleConfig = (config: OcdConsoleConfiguration): Promise<any> => {
        return window.ocdAPI ? window.ocdAPI.saveConsoleConfig(config) : new Promise((resolve, reject) => {reject('Currently Not Implemented')})
    }
}
