...
export namespace GoogleOracleDatabaseCloudExadataInfrastructureProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.GoogleOracleDatabaseCloudExadataInfrastructure, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.GoogleOracleDatabaseCloudExadataInfrastructure>(resource, pH)
