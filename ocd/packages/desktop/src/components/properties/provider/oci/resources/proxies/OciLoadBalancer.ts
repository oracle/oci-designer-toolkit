...
export namespace OciLoadBalancerProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciLoadBalancer, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciLoadBalancer>(resource, pH)
