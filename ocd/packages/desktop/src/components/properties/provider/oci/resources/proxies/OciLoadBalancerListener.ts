...
export namespace OciLoadBalancerListenerProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciLoadBalancerListener, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciLoadBalancerListener>(resource, pH)
