...
export namespace OciSubnetProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciSubnet, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciSubnet>(resource, pH)
