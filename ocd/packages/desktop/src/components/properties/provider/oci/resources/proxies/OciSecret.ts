...
export namespace OciSecretProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciSecret, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciSecret>(resource, pH)
