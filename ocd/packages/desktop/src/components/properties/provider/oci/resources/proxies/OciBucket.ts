...
export namespace OciBucketProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciBucket, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciBucket>(resource, pH)
