...
export namespace OciNatGatewayProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciNatGateway, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciNatGateway>(resource, pH)
