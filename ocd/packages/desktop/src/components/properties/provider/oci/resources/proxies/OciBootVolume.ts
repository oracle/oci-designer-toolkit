...
export namespace OciBootVolumeProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciBootVolume, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciBootVolume>(resource, pH)
