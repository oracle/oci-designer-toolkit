...
export namespace OciNosqlTableProxy {
    ...
    export function proxyResource(ocdDocument: OcdDocument, resource: Model.OciNosqlTable, ocdCache: OcdCacheData) {
        const pH = proxyHandler(ocdDocument, ocdCache)
        const proxyResource = new Proxy<Model.OciNosqlTable>(resource, pH)
