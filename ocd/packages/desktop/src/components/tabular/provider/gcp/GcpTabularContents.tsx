...
import { GcpResource } from "@ocd/model"
import { GcpTabularContentsProps, GcpTabularHeaderProps, GcpTabularResourceProps, GcpTabularRowProps } from "../../../../types/ReactComponentProperties"
import { useContext, useState } from "react"
import { OcdUtils } from "@ocd/core"
import { ConsoleConfigContext } from "../../../../pages/OcdConsole"

export const GcpDefault = ({ ocdDocument, gcpResources, selected }: GcpTabularResourceProps): JSX.Element => {
    console.debug('GcpTabularContents: GcpDefault')
    return (
        <div id='ocd_resource_grid' className='table ocd-tabular-content'>
            <div className='thead ocd-tabular-list-header'><div className='tr'><div className='th'>{gcpResources[selected].length}</div><div className='th'>Name</div><div className='th'>Compartment</div></div></div>
            <div className='tbody ocd-tabular-list-body'>
                {gcpResources[selected].map((r: GcpResource, i: number) => {
...
export const OcdTabularContents = ({ ocdDocument, gcpResources, selected, columnTitles, resourceElements }: GcpTabularContentsProps): JSX.Element => {
    ...
    return (
        <div id='ocd_resource_grid' className='table ocd-tabular-content'>
            ...
            <div className='tbody ocd-tabular-list-body'>
                {gcpResources[selected].sort(sortFunction).map((r: GcpResource, i: number) => {
...
export const OcdTabularHeader = ({columnTitles, gcpResources, resourceElements, selected, sortColumn, sortAscending, onSortClick, displayColumns, setDisplayColumns}: GcpTabularHeaderProps): JSX.Element => {
    const {ocdConsoleConfig, setOcdConsoleConfig} = useContext(ConsoleConfigContext)
    ...
    return (
        <div className='tr' key={`${selected}-tabular-header-row`}>
            <div className='th'>{gcpResources[selected].length}</div>
...
export const OcdTabularRow = ({ocdDocument, gcpResources, index, resource, resourceElements, selected}: GcpTabularRowProps): JSX.Element => {
    const getReferenceDisplayName = (id: string) => {
        const resource = ocdDocument.getResource(id)
        return resource ? resource.displayName : 'Unknown'
    }
