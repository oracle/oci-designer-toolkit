...
import { GoogleResource } from "@ocd/model"
import { GoogleTabularContentsProps, GoogleTabularHeaderProps, GoogleTabularResourceProps, GoogleTabularRowProps } from "../../../../types/ReactComponentProperties"
import { useContext, useState } from "react"
import { OcdUtils } from "@ocd/core"
import { ConsoleConfigContext } from "../../../../pages/OcdConsole"

export const GoogleDefault = ({ ocdDocument, googleResources, selected }: GoogleTabularResourceProps): JSX.Element => {
    console.debug('GoogleTabularContents: GoogleDefault')
    return (
        <div id='ocd_resource_grid' className='table ocd-tabular-content'>
            <div className='thead ocd-tabular-list-header'><div className='tr'><div className='th'>{googleResources[selected].length}</div><div className='th'>Name</div><div className='th'>Compartment</div></div></div>
            <div className='tbody ocd-tabular-list-body'>
                {googleResources[selected].map((r: GoogleResource, i: number) => {
...
export const OcdTabularHeader = ({columnTitles, googleResources, resourceElements, selected, sortColumn, sortAscending, onSortClick, displayColumns, setDisplayColumns}: GoogleTabularHeaderProps): JSX.Element => {
    ...
    return (
        <div className='tr' key={`${selected}-tabular-header-row`}>
            ...
            {displayColumns.map((title: string, i: number) => {return <div className={`th ocd-sortable-column ${sortColumn === resourceElements[i] ? sortAscending ? ascClasses : dscClasses : ''}`} onClick={() => onSortClick(resourceElements[i])} aria-hidden key={`${selected}-tabular-header-row-${OcdUtils.toUnderscoreCase(title)}`}>{title}</div>})}
...
export const OcdTabularRow = ({ocdDocument, googleResources, index, resource, resourceElements, selected}: GoogleTabularRowProps): JSX.Element => {
    const getReferenceDisplayName = (id: string) => {
        const resource = ocdDocument.getResource(id)
        return resource ? resource.displayName : 'Unknown'
    }
