...
export class OciExporter extends OcdProviderExporter {
    ...
    export = (design: OcdDesign): OutputDataStringArray => {
        ...
        if (Object.keys(idTFResourceMap).length > 0) {
            if (design.metadata.separateIdentity) outputData = this.separateIdentityDirectoryExport(design, idTFResourceMap)
            else outputData = this.singleDirectoryExport(design, idTFResourceMap)
        }
        return outputData
    }
    exportOrig = (design: OcdDesign): OutputDataStringArray => {
        // this.design = design
        // Id to Terraform Resource Name Map
        const idTFResourceMap: Record<string, string> = OcdDesign.getOciResources(design).reduce((a, c) => {a[c.id] = c.terraformResourceName; return a}, {} as Record<string, string>)
        console.debug('OcdTerraformExporter: ociExport: idTFResourceMap:', idTFResourceMap)
    ...
    singleDirectoryExport = (design: OcdDesign, idTFResourceMap: Record<string, string>): OutputDataStringArray => {
        let outputData: OutputDataStringArray = {}
        const uniqueFilenames = [...new Set(Object.values(this.resourceFileMap))]
        outputData = {
            "oci_provider.tf": [this.ociProvider()],
    ...
    separateIdentityDirectoryExport = (design: OcdDesign, idTFResourceMap: Record<string, string>): OutputDataStringArray => {
        let outputData: OutputDataStringArray = {}
        const uniqueFilenames = [...new Set(Object.values(this.resourceFileMap))]
        outputData = {
            "identity/oci_provider.tf": [this.ociProvider()],
            "identity/oci_provider_variables.tf": [this.ociProviderVariables()],
            "identity/oci_connection.tfvars": [this.ociProviderTFVars()],
            "identity/oci_metadata.tf": [this.ociMetadata()],
            "identity/oci_identity.tf": [this.autoGeneratedNotice()],
            "identity/oci_user_variables.tf": [this.ociUserVariables(design)],
            "resources/oci_provider.tf": [this.ociProvider()],
            "resources/oci_provider_variables.tf": [this.ociProviderVariables()],
            "resources/oci_connection.tfvars": [this.ociProviderTFVars()],
            "resources/oci_metadata.tf": [this.ociMetadata()],
            ...uniqueFilenames.toSorted(OcdUtils.simpleSort).reduce((a, c) => {a[`resources/${c}`] = [this.autoGeneratedNotice()]; return a}, {} as Record<string, string[]>),
            "resources/oci_user_variables.tf": [this.ociUserVariables(design)],
        }
        // Generate OCI Identity Terraform
        Object.entries(design.model.oci.resources).filter(([k, v]) => this.resourceFileMap[k] === 'oci_identity.tf').forEach(([k, v]) => {
            const className = OcdUtils.toClassName('Oci', k)
            const filename = this.resourceFileMap.hasOwnProperty(k) ? `identity/${this.resourceFileMap[k]}` : this.resourceFileMap['unknown']
            // @ts-ignore
            v.forEach((r: OciResource) => {
                // @ts-ignore 
                const tfResource = new OciTerraformResources[className](r, idTFResourceMap)
        ...
        // Generate OCI Terraform
        Object.entries(design.model.oci.resources).filter(([k, v]) => this.resourceFileMap[k] !== 'oci_identity.tf' || k === 'compartment').forEach(([k, v]) => {
            const className = OcdUtils.toClassName('Oci', k)
            const filename = this.resourceFileMap.hasOwnProperty(k) ? `resources/${this.resourceFileMap[k]}` : this.resourceFileMap['unknown']
            // @ts-ignore
            v.forEach((r: OciResource) => {
                // @ts-ignore 
                const tfResource = new OciTerraformResources[className](r, idTFResourceMap)
...
provider "oci" {
    alias            = "home_region"
    tenancy_ocid     = var.home_tenancy_ocid      == null ? var.tenancy_ocid     : var.home_tenancy_ocid
    user_ocid        = var.home_user_ocid         == null ? var.user_ocid        : var.home_user_ocid
    fingerprint      = var.home_fingerprint       == null ? var.fingerprint      : var.home_fingerprint
    private_key_path = var.home_private_key_path  == null ? var.private_key_path : var.home_private_key_path
    region           = var.home_region            == null ? local.home_region    : var.home_region
}
    `}
...
# -- Home Region Provider Variables

variable "home_tenancy_ocid" {
    type = string
    description = "Home Region OCID of the Tenancy where the defined resources will be created."
    default = null
}
variable "home_user_ocid" {
    type = string
    description = "Home Region OCID of the User who will create the defined resources."
    default = null
}
variable "home_fingerprint" {
    type = string
    description = "Home Region Fingerprint associated with the Private Key File."
    default = null
}
variable "home_private_key_path" {
    type = string
    description = "Home Region Path to the user_ocid users Private Key File."
    default = null
}
variable "home_region" {
    type = string
    description = "Name of the Home Region where the defined resources will be created."
    default = null
}
    `}
...
locals {
    all_services_id          = length(data.oci_core_services.AllRegionServices.services) == 0 ? "" : data.oci_core_services.AllRegionServices.services[0].id
    all_services_cidr_block  = length(data.oci_core_services.AllRegionServices.services) == 0 ? "" : data.oci_core_services.AllRegionServices.services[0].cidr_block
    all_services_destination = length(data.oci_core_services.AllRegionServices.services) == 0 ? "" : data.oci_core_services.AllRegionServices.services[0].cidr_block
}
...
locals {
    objectstorage_services_id          = length(data.oci_core_services.ObjectStorageRegionServices.services) == 0 ? "" : data.oci_core_services.ObjectStorageRegionServices.services[0].id
    objectstorage_services_cidr_block  = length(data.oci_core_services.ObjectStorageRegionServices.services) == 0 ? "" : data.oci_core_services.ObjectStorageRegionServices.services[0].cidr_block
    objectstorage_services_destination = length(data.oci_core_services.ObjectStorageRegionServices.services) == 0 ? "" : data.oci_core_services.ObjectStorageRegionServices.services[0].cidr_block
}
    `}
