...
class BastionProperties extends OkitResourceProperties {
    ...
    buildResource() {
        ...
        // IPv4 CIDR Blocks
        const client_cidr_block_allow_list = this.createInput('ipv4_single_cidr_list', 'Allowed IPv4 CIDR(s)', `${self.id}_client_cidr_block_allow_list`, '', (d, i, n) => {n[i].reportValidity(); self.resource.client_cidr_block_allow_list = n[i].value.replace(' ', '').split(',')})
        ...
        // TTL
        const max_session_ttl_in_seconds = this.createInput('number', 'Session TTL (Seconds)', `${self.id}_max_session_ttl_in_seconds`, '', (d, i, n) => self.resource.max_session_ttl_in_seconds = n[i].value, {min: 1800, max: 10800})
        ...
        // // Advanced
        // const advanced = this.createDetailsSection('Advanced', `${self.id}_advanced_details`)
        // this.append(this.properties_contents, advanced.details)
        // this.advanced_div = advanced.div
        // const advanced_table = this.createTable('', `${self.id}_advanced`)
        // this.advanced_tbody = advanced_table.tbody
        // this.append(this.advanced_div, advanced_table.table)
        // // IPv4 CIDR Blocks
        // const static_jump_host_ip_addresses = this.createInput('ipv4_list', 'Static Jump Host IP(s)', `${self.id}_static_jump_host_ip_addresses`, '', (d, i, n) => {n[i].reportValidity(); self.resource.static_jump_host_ip_addresses = n[i].value.replace(' ', '').split(',')})
