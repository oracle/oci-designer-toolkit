...
*/
console.debug('Loaded NetworkSecurityGroup Properties Javascript');

/*
** Define NetworkSecurityGroup Properties Class
*/
class NetworkSecurityGroupProperties extends OkitResourceProperties {
    ...
    addIngressRule() {
        ...
        this.resource.security_rules.push(rule);
        this.ingress_rule_idx += 1
        this.addIngressRuleHtml(rule, this.ingress_rule_idx);
    }
    deleteIngressRule(id, idx, rule) {
        this.resource.security_rules = this.resource.security_rules.filter((e) => e !== rule)
        $(`#${id}${idx}_row`).remove()
    }
    // Egress Rules
    loadEgressRules() {
